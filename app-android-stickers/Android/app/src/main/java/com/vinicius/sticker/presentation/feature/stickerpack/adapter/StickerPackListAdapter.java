...
public class StickerPackListAdapter extends RecyclerView.Adapter<StickerPackListItemViewHolder> {
   ...
   public StickerPackListItemViewHolder onCreateViewHolder(@NonNull final ViewGroup viewGroup, final int i) {
      ...
      final View stickerPackRow = layoutInflater.inflate(R.layout.sticker_packs_list_item,
          viewGroup,
          false);
   ...
   public void onBindViewHolder(@NonNull final StickerPackListItemViewHolder viewHolder, final int index) {
      ...
      viewHolder.filesizeView.setText(Formatter.formatShortFileSize(context,
          pack.getTotalSize()));
      ...
      viewHolder.container.setOnClickListener(view -> {
         ...
         intent.putExtra(StickerPackDetailsActivity.EXTRA_SHOW_UP_BUTTON,
             true);
      ...
      int actualNumberOfStickersToShow = Math.min(maxNumberOfStickersInARow,
      ...
      setAddButtonAppearance(viewHolder.addButton,
          pack);
   ...
   private void setAddButtonAppearance(ImageView addButton, StickerPack pack) {
      if (pack.getIsWhitelisted()) {
         ...
         addButton.getContext().getTheme().resolveAttribute(android.R.attr.selectableItemBackground,
             outValue,
             true);
