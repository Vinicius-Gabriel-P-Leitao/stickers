...
public class StickerConsumer {
    ...
    public static List<Sticker> getStickersForPack(Context context, String stickerPackIdentifier) {
        ...
        for (Sticker sticker : stickers) {
            final byte[] bytes;
            try {
                bytes = fetchStickerAsset(stickerPackIdentifier, sticker.imageFileName, context.getContentResolver());

                if (bytes.length == 0) {
                    throw new IllegalStateException("Asset file is empty, pack: " + stickerPackIdentifier + ", sticker: " + sticker.imageFileName);
                }

                sticker.setSize(bytes.length);
            } catch (IOException | IllegalArgumentException exception) {
                throw new IllegalStateException(
                        "Asset file doesn't exist. pack: " + stickerPackIdentifier + ", sticker: " + sticker.imageFileName,
                        exception);
            }
        }
    ...
    public static byte[] fetchStickerAsset(
        ...
        try (final InputStream inputStream = contentResolver.openInputStream(
                getStickerAssetUri(identifier, name)); final ByteArrayOutputStream buffer = new ByteArrayOutputStream()) {
