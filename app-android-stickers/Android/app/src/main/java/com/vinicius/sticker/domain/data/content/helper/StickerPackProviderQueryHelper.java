...
// @formatter:off
public class StickerPackProviderQueryHelper {
    private final static String TAG_LOG = StickerPackQueryProvider.class.getSimpleName();

    @NonNull
    private final Context context;

    public StickerPackProviderQueryHelper(@NonNull Context context) {
        this.context = context;
    }

    public Cursor getPackForAllStickerPacks(@NonNull Uri uri, StickerDatabase dbHelper) {
        return StickerPackQueryProvider.getListStickerPackInfo(context, uri, StickerPackQueryProvider.getListStickerPackFromDatabase(dbHelper));
    }
    ...
    public Cursor getCursorForSingleStickerPack(@NonNull Uri uri, StickerDatabase dbHelper) {
        ...
        if (TextUtils.isEmpty(stickerPackIdentifier)) {
            ...
            return StickerPackQueryProvider.getListStickerPackInfo(context, uri, new ArrayList<>());
        }

        try {
            StickerPack stickerPack = StickerPackQueryProvider.getStickerPackFromDatabase(dbHelper, stickerPackIdentifier);
            ...
            return StickerPackQueryProvider.getStickerPackInfo(context, uri, stickerPack);
