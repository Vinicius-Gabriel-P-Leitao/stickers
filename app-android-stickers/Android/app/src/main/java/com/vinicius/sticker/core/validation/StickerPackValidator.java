...
public class StickerPackValidator {
  ...
  public static void verifyStickerPackValidity(
      @NonNull Context context, @NonNull StickerPack stickerPack) throws IllegalStateException {
    ...
    if (stickerPack.identifier.length() > CHAR_COUNT_MAX) {
      throw new PackValidatorException(
          "O identificador do pacote de figurinhas não pode exceder "
              + CHAR_COUNT_MAX
              + " caracteres");
    }
    ...
    if (TextUtils.isEmpty(stickerPack.publisher)) {
      throw new PackValidatorException(
          "O publisher do pacote de figurinhas está vazio, identificador do pacote de figurinhas: "
              + stickerPack.identifier);
    }
    if (stickerPack.publisher.length() > CHAR_COUNT_MAX) {
      throw new PackValidatorException(
          "O publisher do pacote de figurinhas não pode exceder "
              + CHAR_COUNT_MAX
              + " caracteres, identificador do pacote de figurinhas:"
              + stickerPack.identifier);
    }
    if (TextUtils.isEmpty(stickerPack.name)) {
      throw new PackValidatorException(
          "Nome do pacote de figurinhas está vazio, identificador do pacote de figurinhas: "
              + stickerPack.identifier);
    }
    if (stickerPack.name.length() > CHAR_COUNT_MAX) {
      throw new PackValidatorException(
          "O nome do pacote de figurinhas não pode exceder "
              + CHAR_COUNT_MAX
              + " caracteres, identificador do pacote de figurinhas:"
              + stickerPack.identifier);
    }
    if (TextUtils.isEmpty(stickerPack.trayImageFile)) {
      throw new PackValidatorException(
          "O ID do pacote de figurinhas está vazio, identificador do pacote de figurinhas:"
              + stickerPack.identifier);
    }
    ...
    if (!TextUtils.isEmpty(stickerPack.iosAppStoreLink)
      ...
      throw new PackValidatorException(
          "O link da loja de aplicativos iOS deve usar o domínio da loja de aplicativos:"
              + APPLE_STORE_DOMAIN);
    }
    ...
    try {
      ...
      if (stickerAssetBytes.length > TRAY_IMAGE_FILE_SIZE_MAX_KB * KB_IN_BYTES) {
        throw new PackValidatorException(
            "A imagem da thumbnail deve ter menos de "
                + TRAY_IMAGE_FILE_SIZE_MAX_KB
                + " KB, arquivo de thumbnail:"
      ...
      if (bitmap.getHeight() > TRAY_IMAGE_DIMENSION_MAX
        ...
        throw new PackValidatorException(
            "A altura da thumbnail deve estar entre"
                + TRAY_IMAGE_DIMENSION_MIN
                + " e "
                + TRAY_IMAGE_DIMENSION_MAX
                + " pixels, a altura atual da imagem da bandeja é"
                + bitmap.getHeight()
                + ", arquivo: "
                + stickerPack.trayImageFile);
      }
      if (bitmap.getWidth() > TRAY_IMAGE_DIMENSION_MAX
        ...
        throw new PackValidatorException(
            "A largura da thumbnail deve estar entre "
                + TRAY_IMAGE_DIMENSION_MIN
                + " e "
                + TRAY_IMAGE_DIMENSION_MAX
                + " pixels, a largura atual da imagem da bandeja é "
                + bitmap.getWidth()
                + ", arquivo: "
                + stickerPack.trayImageFile);
      }
      ...
      throw new PackValidatorException(
          "Não é possível abrir a thumbnail, " + stickerPack.trayImageFile, exception);
    }
    ...
    if (stickers.size() < STICKER_SIZE_MIN || stickers.size() > STICKER_SIZE_MAX) {
      throw new PackValidatorException(
          "A quantidade de figurinhas do pacote deve estar entre 3 a 30, atualmente tem"
              + stickers.size()
              + ", identificador do pacote de figurinhas: "
              + stickerPack.identifier);
    }
  }
  ...
  private static void checkStringValidity(@NonNull String string) {
    ...
    if (!string.matches(pattern)) {
      throw new IllegalStateException(
          string
              + " contém caracteres inválidos, os caracteres permitidos são de a a z, A a Z, _, ' - . e caractere de espaço");
    }
    if (string.contains("..")) {
