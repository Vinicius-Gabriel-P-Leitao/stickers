...
import com.vinicius.sticker.domain.data.content.helpers.StickerProviderQueryHelper;
...
public class StickerContentProvider extends ContentProvider {
  ...
  public Cursor query(
    ...
    StickerProviderQueryHelper stickerProviderQueryHelper =
        new StickerProviderQueryHelper(getContext());

    if (code == METADATA_CODE) {
      return stickerProviderQueryHelper.getPackForAllStickerPacks(uri, dbHelper);
    } else if (code == METADATA_CODE_FOR_SINGLE_PACK) {
      return stickerProviderQueryHelper.getCursorForSingleStickerPack(uri, dbHelper);
    } else if (code == METADATA_CODE_ALL_STICKERS) {
      return stickerProviderQueryHelper.getStickersForAStickerPack(uri, dbHelper);
    } else {
  ...
  public static Uri getSingleStinckerPackUri(String packIdentifier) {
    return Uri.withAppendedPath(AUTHORITY_URI, packIdentifier);
  }
  ...
  private AssetFileDescriptor getImageFiles(Uri uri) throws IllegalArgumentException {
    ...
    if (stickerFile.exists() && stickerFile.isFile()) {
      try {
        ...
        Log.e(
            getContext().getPackageName(),
            "Erro ao abrir stickerFile: " + stickerFile.getAbsolutePath(),
            exception);
      }
    }
