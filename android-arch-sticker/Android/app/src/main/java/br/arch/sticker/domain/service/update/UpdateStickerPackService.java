...
public class UpdateStickerPackService {
    ...
    public boolean updateStickerFileName(String stickerPackIdentifier, String newName) throws UpdateStickerException {
        ...
        Log.d(TAG_LOG, applicationTranslate.translate(R.string.debug_update_sticker_pack_name, stickerPackIdentifier, newName).get());

        if (updateStickerPackRepo.updateStickerPackName(stickerPackIdentifier, newName)) {
            return true;
        }

        throw new UpdateStickerException(applicationTranslate.translate(R.string.error_update_sticker_pack).log(TAG_LOG, Level.ERROR).get(),
    ...
    public boolean cleanStickerPackUrl(String stickerPackIdentifier) throws UpdateStickerException {
        if (stickerPackIdentifier.isEmpty()) {
            Log.w(TAG_LOG, applicationTranslate.translate(R.string.warn_invalid_parameters_clean_urls).log(TAG_LOG, Level.WARN).get());
            return false;
        }

        Log.d(TAG_LOG, applicationTranslate.translate(R.string.debug_update_sticker_pack_name, stickerPackIdentifier).get());

        if (updateStickerPackRepo.cleanStickerPackUrl(stickerPackIdentifier)) {
            return true;
        }

        throw new UpdateStickerException(applicationTranslate.translate(R.string.error_update_urls_sticker_pack).log(TAG_LOG, Level.WARN).get(),
                ErrorCode.ERROR_EMPTY_STICKERPACK
        );
    }
}
